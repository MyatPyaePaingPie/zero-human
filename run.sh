...
set -euo pipefail
cd "$(dirname "$0")"
for name in REPLAY_API_KEY RC_PAYLINK_DEFAULT RC_PAYLINK_FULL_REALITY_CHECK ZEROHUMAN_STRIPE_WRITE_KEY ZEROHUMAN_STRIPE_RESTRICTED_KEY ZEROHUMAN_STRIPE_PUBLISHABLE_KEY TERAC_API_KEY GROQ_API_KEY OPENAI_API_KEY RC_ENVELOPE_SECRET; do
  if [ -z "${!name:-}" ]; then
